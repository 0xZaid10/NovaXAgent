...
function evaluateRules(config, i, ind, candles) {
    ...
    if (x.macd.enabled) {
        const m = ind.macd(x.macd.fast, x.macd.slow, x.macd.signal)[i - x.macd.slow];
        if (!m || m.MACD <= m.signal)
            return false;
    }
    if (x.bollingerBands.enabled) {
        const bb = ind.bb(x.bollingerBands.period, x.bollingerBands.deviation)[i - x.bollingerBands.period];
